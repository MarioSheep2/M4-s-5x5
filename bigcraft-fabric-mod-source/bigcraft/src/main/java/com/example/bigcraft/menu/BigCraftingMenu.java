...
public class BigCraftingMenu extends AbstractContainerMenu {
    ...
    public static final int GRID_SIZE = GRID_WIDTH * GRID_HEIGHT; // 25
    ...
    /** Client-side constructor used by the screen. */
    public BigCraftingMenu(int syncId, Inventory playerInventory) {
        ...
        // Output slot, to the right of the grid.
    ...
    private void slotsChangedInternal() {
        if (this.player != null && !this.player.level().isClientSide()) {
            this.resultSlot.updateResult(this.player.level().getRecipeManager(), this.player.level());
        }
    }
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        // Simple shift-click support: move between the grid+output block and the
        // player inventory block. Keep it straightforward rather than feature-complete.
        ...
        if (index < gridAndResultSlots) {
            // Moving out of the crafting area into the player inventory.
            if (!this.moveItemStackTo(original, gridAndResultSlots, this.slots.size(), true)) {
                return ItemStack.EMPTY;
            }
        } else {
            // Moving from player inventory into the 5x5 grid (not the result slot).
    ...
    public void removed(Player player) {
        ...
        // Return any items left in the grid to the player, like vanilla crafting does.
